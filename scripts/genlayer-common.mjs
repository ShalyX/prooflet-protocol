...
export function config({ requireContract = true, requireKey = true } = {}) {
  ...
  const privateKey = process.env.GENLAYER_PRIVATE_KEY || process.env.TREASURY_PRIVATE_KEY;
  const missing = [requireContract && !contractAddress && "GENLAYER_CONTRACT_ADDRESS", requireKey && !privateKey && "GENLAYER_PRIVATE_KEY (or TREASURY_PRIVATE_KEY)"].filter(Boolean);
