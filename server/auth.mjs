...
export function storeApiKey(db, ownerType, ownerId, apiKey, createdAt = new Date().toISOString()) {
  db.prepare(`
...
export function authenticate(db, request, ownerType, ownerId) {
...
export function storeAdjudicatorApiKey(db, adjudicatorId, apiKey, scopes, createdAt = new Date().toISOString()) {
...
export function authenticateAdjudicator(db, request, requiredScope) {
  const apiKey = readApiKey(request);
  if (!apiKey) return null;
  const row = db.prepare(`SELECT k.*,a.status FROM adjudicator_api_keys k JOIN adjudicators a USING(adjudicator_id)
